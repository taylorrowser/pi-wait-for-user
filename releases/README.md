...
9. Run `release-metadata.mjs promote` with a higher Channel sequence. It verifies trust, signatures, manifest digest, and both replay checkpoints, then generates `trust-state.json`, `channel-state.json`, the Channel, `artifact-manifest.json`, `SHA256SUMS`, archive metadata, and temporary `active.json` compatibility projection. Generate `installation-receipt-<platform>.json` for every managed macOS/Linux platform from that verified manifest. Published receipt projections use `$MANAGED_DATA_ROOT/downstream-releases/<release-id>` to state the portable ownership scope. The installer does not trust that downloaded projection; it independently derives the absolute platform-native path and creates stricter local runtime receipts from the same verified Release Manifest before Activation.
