# Pi Wait for User · `pi-v0.81.1-patch.13`

Managed Installation release for macOS Apple Silicon, Linux ARM64/x64, and Windows ARM64/x64.

## What changed

- Native Windows release smoke now preserves the conformance command exit code, displays at most 100 captured lines, and requires an exact `Deferred conformance passed (8/8)` output line without PowerShell array-filter truthiness.
...
```text
https://github.com/taylorrowser/pi-wait-for-user/releases/download/pi-v0.81.1-patch.13/install.sh
```
...
The public `pi-v0.81.1-patch.12` tag is an unpublished failed identity: run `30224148376` stopped at native Windows smoke before production approval, so it has no GitHub Release, signature, promotion branch, or Channel sequence 3. The tag remains immutable and is never moved, deleted, reused, or rerun. `pi-v0.81.1-patch.13` is a new candidate identity; `pi-v0.81.1-patch.11` remains the currently published immutable release until a later authorized promotion completes.
