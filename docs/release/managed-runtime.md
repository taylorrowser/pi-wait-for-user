...
The release `install.sh` is the reviewed stage-0 bootstrap. It embeds the #58-provisioned production root public key, verifies root-signed trust plus delegated Channel/Release Manifest signatures, selects the exact signed Manager Release and platform Downstream Release artifacts, verifies signed size/digest before executing Manager Release code, and invokes the same installer-pinned lifecycle seam as `scripts/managed-installer.mjs`. Neither the trust anchor nor verification clock is caller-selectable. Command Ownership is refused for a pair created through the low-level caller-key activation seam, and the stable Dispatcher embeds the installer-pinned root-key configuration. The checksum/attestation-first path independently verifies `install.sh` and the embedded public-key fingerprint before invocation. Only explicit `--manage-pi` claims Command Ownership; without it a signed installation publishes only `pi-wait-for-user`. It remains side-by-side until either `install.sh --manage-pi` is selected or the user runs:
...
Automatic post-activation retention keeps the active pair, previous pair, every Pinned Release pair, and every live-leased pair. `pi managed pin [release-id]` and `unpin [release-id]` default to the active release. `pi managed prune` removes older receipt-proven pairs and records leased pairs for retry. A durable pair-cleanup claim closes lease acquisition before the final live-lease scan, so a delayed launch cannot lease a pair while its payload is being tombstoned. Cleanup starts only after the Activation switch, and a later activation, prune, or explicit cleanup pass converges deferred work after leases end or interruption at any tombstone/receipt boundary.
...
Issue #63 release-level evidence, including the synthetic unchanged-upstream patch transition and the macOS/Linux platform smoke matrix, is indexed in [`managed-release-acceptance.md`](managed-release-acceptance.md).
