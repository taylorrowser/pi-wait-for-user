...
This is the release-level traceability record for GitHub issues #42, #63, #64, #80, #82, #87, #89, and #91 and the accepted [Managed Installation design](../design/managed-installation.md). Observable behavior is tested at the filesystem/CLI, signed-metadata, release-bundle, and workflow boundaries; private helpers are not acceptance seams.
...
## Question Tool retained-draft editing (#42)

| #42 requirement | Automated evidence |
| --- | --- |
| First Backspace/Delete on a selected retained custom row enters editing without mutating its draft | `packages/question-tool/test/question-form.test.mjs` drives the public Question Form component from navigation into editing and asserts the retained `abc` value remains visible |
| Later Backspace/Delete edits normally after editing is active | The same focused form test applies a second Backspace, submits, and asserts the custom Response is `ab`; the Delete case independently preserves and submits `abc` |
| Package behavior and release identity stay aligned | Question Tool `0.1.5`, `packages/question-tool/README.md`, the patch.14 candidate manifest, and release notes declare the same two-step interaction contract |
...
| Preserve strict rejection of unrelated missing and malformed/unknown drift | `packages/ai/test/model-data-validation.test.ts` exercises public `hydrateModelDataStructure`: the exact four withdrawals reconcile, while missing `fireworks/accounts/fireworks/models/glm-5p2` and an `unknown-api` mismatch both fail closed; `model-catalog-retirement` pins this regression in the patch.14 fixture gate |
| Preserve immutable published patch.13 while preparing patch.14 with the same Manager Release and patch series | The patch.13 directory, tag, manifest, and public release remain unchanged; `releases/pi-v0.81.1-patch.14/manifest.json`, fixture gate, release notes, package/bootstrap projections, and this traceability record select a distinct patch.14 candidate with `manager-v3` and the ordered 20-patch series |
...
| Failed public patch.12 remains immutable while later releases use new identities | The published patch.13 release and `releases/pi-v0.81.1-patch.14/RELEASE_NOTES.md` preserve patch.12 as an unpublished failed identity; package, bootstrap, manifest, fixture gate, install examples, and notes select the distinct `pi-v0.81.1-patch.14` candidate with `manager-v3` |
