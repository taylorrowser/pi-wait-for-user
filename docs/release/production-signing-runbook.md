...
1. `release-candidate` has no production signing secret. After building the unsigned manifest, it signs only with the explicitly public checked-in fixture authority and calls the same executable receipt-projection boundary as production. The mandatory preflight verifies that authority's root and delegated SPKI fingerprints plus canonical trust-envelope digest, workspace-relative and absolute manifest loading, exact `darwin-arm64`/`linux-arm64`/`linux-x64`/`windows-arm64`/`windows-x64` receipt outputs, and fail-closed missing/malformed probes. It writes the concise workflow summary before preserving the passing machine report as the final durable success evidence; a summary-write failure leaves no passing report. The candidate cannot satisfy the protected job dependency without that success.
