...
test("the release candidate input verifies every pinned identity without becoming an active Channel pointer", () => {
  ...
  assert.match(
    result.stdout,
    new RegExp(`Verified ${releaseCandidateId.replaceAll(".", "\\.")}: Pi v0\\.81\\.1, ${releaseCandidateInput.patches.length} patches, Question Tool ${releaseCandidateInput.questionTool.version.replaceAll(".", "\\.")}`),
  );
});
