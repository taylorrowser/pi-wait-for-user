...
ordinary/queued user-input Interruption, branch/compaction preservation, and
non-TUI projection slices for issues #13 through #20. It adds the versioned tool
capability, fail-closed downstream session identity, persisted deferred-batch
marker, neutral markerless/deferred/partial/continuation AgentSession snapshot,
...
session-local request identity reuse, exact boundary copying,
unresolved-owner compaction pinning, complete deferred-state events, RPC
inspection/advancement/error contracts, and privacy-safe JSON/print Waiting
State output for Pi v0.81.1.
