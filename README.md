...
The active release is **`pi-v0.81.1-patch.3`**. It combines:
...
Download `SHA256SUMS` and the matching asset from the [active GitHub release](https://github.com/taylorrowser/pi-wait-for-user/releases/tag/pi-v0.81.1-patch.3):
...
```bash
# Verify
curl -fsSL https://github.com/taylorrowser/pi-wait-for-user/releases/download/pi-v0.81.1-patch.3/install.sh | sh -s -- verify

# Uninstall this release
curl -fsSL https://github.com/taylorrowser/pi-wait-for-user/releases/download/pi-v0.81.1-patch.3/install.sh | sh -s -- uninstall
```
...
```bash
gh release download pi-v0.81.1-patch.3 \
  --pattern 'pi-wait-for-user-pi-v0.81.1-patch.3.tgz'
tar -xzf pi-wait-for-user-pi-v0.81.1-patch.3.tgz
