...
Download `SHA256SUMS` and the matching asset from the [active GitHub release](https://github.com/taylorrowser/pi-wait-for-user/releases/tag/pi-v0.81.1-patch.6):
...
```bash
# Verify
curl -fsSL https://github.com/taylorrowser/pi-wait-for-user/releases/download/pi-v0.81.1-patch.6/install.sh | sh -s -- verify

# Uninstall this release
curl -fsSL https://github.com/taylorrowser/pi-wait-for-user/releases/download/pi-v0.81.1-patch.6/install.sh | sh -s -- uninstall
```
...
```bash
gh release download pi-v0.81.1-patch.6 \
  --pattern 'pi-wait-for-user-pi-v0.81.1-patch.6.tgz'
tar -xzf pi-wait-for-user-pi-v0.81.1-patch.6.tgz
...
The release package also contains the `manager-v1` local managed runtime: the stable stage-0 dispatcher, atomic Activation engine, immutable pair receipts, lifecycle lock, process leases, recovery/disable operations, layered verification, and the macOS/Linux side-by-side/explicit-ownership installer engine. See [`docs/release/managed-runtime.md`](docs/release/managed-runtime.md). Production Channel/bootstrap wiring and network update UX are completed by the release-integration follow-ups; the currently published fast installer remains side-by-side.
